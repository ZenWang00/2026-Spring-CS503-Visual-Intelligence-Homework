...
class Mlp(nn.Module):
    ...
    def __init__(self, 
        ...
        self.act = nn.GELU()
        self.fc2 = nn.Linear(hidden_features, out_features, bias=bias) # TODO

    def forward(self, x: torch.Tensor) -> torch.Tensor:

        return self.fc2(self.act(self.fc1(x))) # TODO
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        ...
        self.attn = Attention(dim=dim, head_dim=head_dim, qkv_bias=use_bias, proj_bias=use_bias) # TODO
...
class TransformerDecoderTrunk(nn.Module):
    ...
    def forward(
        ...
        for blk in self.blocks:
            x = blk(x, context, sa_mask=sa_mask, xa_mask=xa_mask)
        return x # TODO