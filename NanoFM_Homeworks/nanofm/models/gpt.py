...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding = nn.Embedding(vocab_size, dim, padding_idx=padding_idx if padding_idx >= 0 else None) # TODO: Define the input embedding layer
        self.positional_embedding = nn.Parameter(torch.empty(max_seq_len, dim)) # TODO: Define the learnable positional embedding
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        pos = self.positional_embedding[:L, :].unsqueeze(0)
        x = x + pos
        ...
        mask = torch.tril(torch.ones((L, L), device=x.device, dtype=torch.bool)).unsqueeze(0)
