...
class MaskGIT(nn.Module):
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        k_per_sample = torch.randint(1, L + 1, (B,), device=seq.device)
        rand = torch.rand(B, L, device=seq.device)
        rank = rand.argsort(dim=1).argsort(dim=1)
        mask = rank < k_per_sample.unsqueeze(1)
        return mask
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            confidence = masked_logits.max(dim=-1)[0]
            ...
            top_k_indices = confidence.argsort(descending=True)[:k]
