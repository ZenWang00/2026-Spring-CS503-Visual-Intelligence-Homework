...
class VQACollator(object):  # Visual Question Answering Collator
    ...
    def __call__(self, batch):
        ...
        original_lengths = [
            len(self.tokenizer.encode(seq, add_special_tokens=True))
            for seq in input_sequences
        ]
        # Then iterate over each sample i of the batch and handle two cases:
        for i in range(len(batch)):
            # Get the length of the question for this sample
            question_length = len(self.tokenizer.encode(texts[i], add_special_tokens=True))
            ...
            first_token_pos = attention_mask[i].nonzero(as_tuple=False)[0].item() # the first 1 (non-zero value) in the attention mask for a given batch sample marks the first non-padding token
            ...
            question_end = min(first_token_pos + question_length - 1, labels.size(1))
            labels[i, attention_mask[i] == 0] = -100
            labels[i, :question_end] = -100 # update labels for padding and question part to -100
