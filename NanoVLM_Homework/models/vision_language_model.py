...
class VisionLanguageModel(nn.Module):
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        if image_embd_encoder.dim() == 2:
            image_embd_encoder = image_embd_encoder.unsqueeze(1)
        ...
        if attention_mask is not None:
            image_attention_mask = torch.ones(
                (attention_mask.size(0), img_seq_len),
                dtype=attention_mask.dtype,
                device=attention_mask.device,
            )  # hint: we want all image tokens to be attended
        ...
        for i in range(max_new_tokens):
            ...
            if torch.all(next_token.squeeze(1) == 2): # vii)
                break
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        if image_embd.dim() == 2:
            image_embd = image_embd.unsqueeze(1)
        ...
        model_out, past_key_values = self.decoder.forward_kv(
            combined_embd, past_key_values=None, attention_mask=attention_mask
        )
        ...
        for i in range(1, max_new_tokens):
            ...
            model_out, past_key_values = self.decoder.forward_kv(
                next_embd, past_key_values=past_key_values, attention_mask=attention_mask
            ...
            if torch.all(next_token.squeeze(1) == 2): # stop the generation loop if the generated token == 2 (token id 2 is the EOS token we used during training)
                break
